...
public class BlueTests extends Activity {
   ...
   // private static TextView mTitle;
     ...
     * classes.
     */
    public static final String LOG_TAG = "Bluetooth CMD";
...
//    };
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mSerialService = new BluetoothServices(this, mHandlerBT, mEmulatorView);
    ...
    public synchronized void onResume() {
        ...
        if (!mEnablingBT) { // If we are turning on the BT we cannot check if it's enable
            ...
            if (mSerialService != null) {
                ...
                if (mSerialService.getState() == BluetoothServices.STATE_NONE) {
...
*/
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.connect:
                ...
                if (getConnectionState() == BluetoothServices.STATE_NONE) {
                    // Launch the DeviceListActivity to see devices and do scan
                    Intent serverIntent = new Intent(this, BlueDeviceList.class);
                    startActivityForResult(serverIntent, REQUEST_CONNECT_DEVICE);
                }
                else
                if (getConnectionState() == BluetoothServices.STATE_CONNECTED) {
                    mSerialService.stop();
                    mSerialService.start();
                }
                return true;
            ...
            case R.id.return_to_app:
                startActivity(new Intent (BlueTests.this, Main.class));
                finish();
                return true;
        }
        return false;
    ...
    private void doPreferences() {
        startActivity(new Intent(this, BluePreferences.class));
    }
    ...
    private void showAboutDialog() {
        mAboutDialog = new Dialog(BlueTests.this);
...
class TerminalEmulator {
    ...
    public void append(byte[] buffer, int base, int length) {
        for (int i = 0; i < length; i++) {
            ...
            try {
                if (BlueTests.LOG_CHARACTERS_FLAG) {
                    char printableB = (char) b;
                    if (b < 32 || b > 126) {
                        printableB = ' ';
                    }
                    Log.w(BlueTests.LOG_TAG, "'" + Character.toString(printableB) + "' (" + Integer.toString(b) + ")");
                }
                ...
                Log.e(BlueTests.LOG_TAG, "Exception while processing character "
    ...
    private void selectGraphicRendition() {
        for (int i = 0; i <= mArgIndex; i++) {
            ...
            if (code == 0) { // reset
                ...
                if (BlueTests.LOG_UNKNOWN_ESCAPE_SEQUENCES) {
                    Log.w(BlueTests.LOG_TAG, String.format("SGR unknown code %d", code));
                }
            }
        }
    }
